...
namespace Confab.Shared.Infrastructure.Exceptions
{
    internal class ExceptionCompositionRoot : IExceptionCompositionRoot
    {
        ...
        public ExceptionResponse Map(Exception exception)
        {
            ...
            var noDefaultMappers = mappers.Where(x => x is not ExceptionToResponseMapper);
            var result = noDefaultMappers.Select(x => x.Map(exception)).SingleOrDefault(x => x is not null);
